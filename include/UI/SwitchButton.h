...
class SwitchButton :public Button
{
  protected:
    ...
    sf::RectangleShape info;//информация о выбранной настройке
    sf::Text infoText;
  ...
  public:
    ...
    SwitchButton(const sf::Vector2f &size, const std::string &text, const sf::Font &font,
                 const unsigned int characterSize,const std::map<std::string, std::function<void(void)>> &callbacks): SwitchButton(sf::Vector2f(0,0),size,text, font, characterSize, callbacks){}
