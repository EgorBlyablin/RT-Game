...
{
  private:
    std::pair<std::function<void(void)>, std::function<void(void)>> callbacks;//первый элемент - вкл;

    bool isTurnedOn = false;

  public:
    TurnOnOffButton(const sf::Vector2f &position, const sf::Vector2f &size, const std::string &text,
